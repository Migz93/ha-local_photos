...
This document describes all configuration options and settings available in the Local Photos custom integration.
...
```yaml
service: local_photos.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: local_photos.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
- [GitHub Issues](https://github.com/Migz93/ha-local_photos/issues) - Report problems
