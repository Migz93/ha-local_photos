...
This document describes the technical architecture of the Local Photos custom component for Home Assistant.
...
│   ├── __init__.py          # Exports LocalPhotosDataUpdateCoordinator
...
│   ├── __init__.py          # Exports LocalPhotosEntity
...
- `base.py` - Main coordinator class (`LocalPhotosDataUpdateCoordinator`)
...
**Key class:** `LocalPhotosDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
**Key class:** `LocalPhotosApiClient`
...
- `LocalPhotosConfigFlowHandler` (main flow)
- `LocalPhotosOptionsFlow` (options)
...
**Key class:** `LocalPhotosEntity` (in `entity/base.py`)
...
2. `LocalPhotosEntity` for common functionality
...
1. Create directory: `custom_components/local_photos/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `LocalPhotosEntity`
