...
- **Domain:** `local_photos`
- **Title:** Local Photos
- **Repository:** Migz93/ha-local_photos

**Key directories:**

- `custom_components/local_photos/` - Main integration code
...
- Integration logs: `custom_components.local_photos: debug` in `config/configuration.yaml`
...
- Use the domain `local_photos` for all DOMAIN references
- Prefix all integration-specific classes with `LocalPhotos`
- Use "Local Photos" as the display title
...
- Inherit from platform base + `LocalPhotosEntity`
...
- `tests/` mirrors `custom_components/local_photos/` structure
...
- Any rename of `local_photos` → `ha_integration_domain`
- Any rename of `LocalPhotos` → `IntegrationBlueprint`
- Any rename of `Local Photos` → `Integration Blueprint`
